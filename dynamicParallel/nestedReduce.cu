...
// iDim was used to remember global offset since child gridDim not the same as pararent
// all child was spawned by block0, so odata not need anymore
__global__ void gpuRecursiveReduceNosyncMultiGradChild (int *g_idata, int *g_odata,
                                          unsigned int isize, unsigned int iDim)
{
    // set thread ID
    unsigned int tid = threadIdx.x;
    ...
    {
        gpuRecursiveReduceNosyncMultiGradChild<<<gridDim.x, isize/2>>>(g_idata, g_odata, isize/2, iDim);
...
int main(int argc, char **argv)
{
    ...
    // gpu nested reduce kernel without sync
    CHECK(cudaMemcpy(d_idata, h_idata, bytes, cudaMemcpyHostToDevice));
    iStart = seconds();
    ...
    // gpu nested reduce kernel without sync and multi-grid children
    CHECK(cudaMemcpy(d_idata, h_idata, bytes, cudaMemcpyHostToDevice));
    iStart = seconds();
    gpuRecursiveReduceNosyncMultiGradChild<<<grid, block.x/2>>>(d_idata, d_odata, block.x/2, block.x);
    CHECK(cudaDeviceSynchronize());
    CHECK(cudaGetLastError());
    iElaps = seconds() - iStart;
    CHECK(cudaMemcpy(h_odata, d_odata, grid.x * sizeof(int),
    ...
    printf("gpu nestedMulti\t\telapsed %f sec gpu_sum: %d <<<grid %d block %d>>>\n",
           iElaps, gpu_sum, grid.x, block.x);
